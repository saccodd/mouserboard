...
parasails.registerComponent('jsTimestamp', {
  ...
  beforeMount: function() {
    ...
    if(this.format === undefined) {
      ...
      if(!_.contains(['calendar','calendar-time', 'timeago'], this.format)) { throw new Error('Unsupported `format` ('+this.format+') passed in to the JS timestamp component! Must be either \'calendar\' or \'timeago\'.'); }
    ...
    if(this.formatType === 'calendar-time') {
      this.formattedTimestamp = moment(this.at).format('DD MMM YYYY') + ' at '+ moment(this.at).format('HH:MM');
      if (this.short) {
        throw new Error('Invalid usage of <js-timestamp>:  Cannot use `short="true"` at the same time as `format="calendar"`.');
      }
    }
